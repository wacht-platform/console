...
import { Field, FieldGroup, Label, Description, Fieldset } from "@/components/ui/fieldset";
...
export default function JWTTemplateCreateUpdatePage() {
    ...
    const { createJWTTemplate, isCreatingJWTTemplate, updateJWTTemplate, isUpdatingJWTTemplate, jwtTemplates, deleteJWTTemplate, isDeletingJWTTemplate } = useDeploymentJWTTemplates();
        ...
        if (value !== undefined) {
            setFormData(prev => {
                try {
                    ...
                    return { ...prev, template: JSON.stringify(parsed, null, 2) };
                } catch (e) {
    ...
    const validateForm = () => {
        ...
        try {
            ...
            setValidationError("Invalid JSON format in claims template");
            return false;
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setValidationError(`Failed to ${isEditMode ? 'update' : 'create'} JWT template`);
            console.error(error);
        }
    };
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <Heading>{isEditMode ? 'Update Template' : 'Create New Template'}</Heading>
                        ...
                        {isCustomSigningKey && (
                            <div className="space-y-4">
                                <Field>
                                    ...
                                    <Listbox name="signing_algorithm" value={signingAlgorithm} onChange={handleAlgorithmChange}>
                        ...
                        <Field>
                            ...
                            <div className="relative mt-2">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        navigator.clipboard.writeText(selectedDeployment?.backend_host || "");
                                    }}
                                >
                                    Copy
                                </button>
                        ...
                        <Field>
                            ...
                            <div className="relative mt-2">
                                ...
                                <button
                                    className="absolute right-2 top-1/2 transform -translate-y-1/2 px-2 py-1 text-sm text-indigo-600 bg-indigo-50 rounded z-10"
                                    onClick={() => {
                                        navigator.clipboard.writeText(`https://${selectedDeployment?.backend_host}/.well-known/jwks.json`);
                                    }}
                                >
                                    Copy
                                </button>
                    ...
                    <FieldGroup>
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                            ...
                            <div className="md:col-span-4 space-y-4">
                                <h3 className="text-sm font-medium text-gray-700">Insert shortcodes</h3>
                                <div className="flex flex-wrap gap-2">
                                    <button onClick={() => insertShortcode("user.id")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.id</button>
                                    <button onClick={() => insertShortcode("user.external_id")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.external_id</button>
                                    <button onClick={() => insertShortcode("user.first_name")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.first_name</button>
                                    <button onClick={() => insertShortcode("user.last_name")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.last_name</button>
                                    <button onClick={() => insertShortcode("user.full_name")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.full_name</button>
                                    <button onClick={() => insertShortcode("user.username")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.username</button>
                                    <button onClick={() => insertShortcode("user.created_at")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.created_at</button>
                                    <button onClick={() => insertShortcode("user.updated_at")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.updated_at</button>
                                    <button onClick={() => insertShortcode("user.last_sign_in_at")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.last_sign_in_at</button>
                                    <button onClick={() => insertShortcode("user.primary_email_address")} className="text-left px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm w-fit">user.primary_email_address</button>
                                </div>
                            </div>
                        </div>
                    </FieldGroup>
