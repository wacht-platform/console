...
// Access-page variant of the vanity embed shell: mints its ticket against the
// deployment-scoped (backend-kind) endpoint — real deployment, honors actor_id —
// instead of the console preview endpoint. Kept separate so the shared
// VanityEmbedShell (used by the standalone preview pages) stays untouched.

type VanityKind = "webhook" | "api-auth";

interface AccessVanityShellProps {
    kind: VanityKind;
    appSlug?: string;
}
...
async function createSessionTicket(
    ...
    const response = await apiClient.post<TicketResponse>(
        `/deployments/${deploymentId}/access/session-ticket`,
        body,
    );
    return response.data;
}

export function AccessVanityShell({ kind, appSlug }: AccessVanityShellProps) {
...
interface AccessVanityFrameProps {
    loading: boolean;
    error: string | null;
    src: string | null;
    kind: VanityKind;
