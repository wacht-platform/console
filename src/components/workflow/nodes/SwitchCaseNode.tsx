...
const SwitchCaseNode = memo(({ data }: SwitchCaseNodeProps) => {
  ...
  return (
    <div className="relative border border-dashed border-indigo-400 bg-indigo-50 text-indigo-800 dark:border-indigo-500 dark:bg-indigo-900/20 dark:text-indigo-300 rounded p-0.5 text-center cursor-grab transition-colors hover:bg-indigo-100 dark:hover:bg-indigo-900/30 w-12 h-8">
      <Handle
        type="target"
        position={Position.Top}
        className="w-1.5 h-1.5 !bg-indigo-500 !border-0"
      />

      <div className="font-medium text-xs truncate leading-tight">{data.label}</div>
      <div className="text-xs opacity-75 leading-tight">Conditional logic</div>
      ...
      {data.default_case && (
        <Handle
          ...
          position={Position.Bottom}
          id="default"
          className="w-1.5 h-1.5 !bg-gray-500 !border-0"
          style={{
